...
proc ::nntp::_cget {name option} {
	...
	return $data(binary)
    } elseif {[string equal $option -debug]} {
	return $data(debug)
    } else {
	return -code error \
		"Illegal option \"$option\", expected \"-binary\" or \"-debug\""
    }
}
...
proc ::nntp::_configure {name args} {
    ...
    if {[llength $args] == 0} {
	return [list -binary $data(binary) -debug $data(debug)]
    }
    if {[llength $args] == 1} {
	...
	if {[string equal $o -binary]} {
	    ...
	    return -code error \
		    "Illegal option \"$o\", expected \"-binary\" or \"-debug\""
	}
    }
    return {}
}
